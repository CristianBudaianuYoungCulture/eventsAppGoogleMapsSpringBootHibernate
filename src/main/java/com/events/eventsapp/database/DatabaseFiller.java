...
/**
 * This class provides several method for filling database with initial and/or
 * initial data. For production use productionFill() method. For testing and
 * developing purposes use testingFill() method.
 */
@Component
public class DatabaseFiller implements ApplicationRunner {
	...
	public void productionFill() {
		...
		EventCategoryModel eventCategoryModel1 = iEventCategoryService
				.findEventCategoryModelByName("Parties & social life");
		EventCategoryModel eventCategoryModel2 = iEventCategoryService
				.findEventCategoryModelByName("Automotive");
		EventCategoryModel eventCategoryModel3 = iEventCategoryService
				.findEventCategoryModelByName("Concert");
	...
	/**
	 * Method calls productionFill() method then populates database with example
	 * data.
	 */
	public void testingFill() {
		...
		testAdminModel.setRoles(
				new HashSet<RoleModel>(Arrays.asList(iRoleService.findRoleByName("ADMIN"))));
		...
		testUser2Model.getUserDetailsModel()
				.setBirthdate(DateAndTimeUtil.getTimestamp(1981, 2, 3, 0, 0));
