...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/admin/**").access("hasRole('ADMIN')").and()
				.authorizeRequests()
				.antMatchers("/user/**", "/addEvent", "/eventManager", "/timeLine", "/mainTimeLine",
						"/userDetailsView", "/userDetailsManager", "/contactsView", "/messageView")
				.access("hasAnyAuthority('ADMIN','USER')").and().formLogin().loginPage("/login")
